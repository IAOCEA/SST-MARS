...
def main():
    ...
    plt.subplot(2,2,1)
    plt.imshow((im-np.median(im))/sm,cmap=cmap,vmin=vmin,vmax=vmax,origin='lower',aspect='auto')
    plt.title('Model')
    plt.subplot(2,2,2)
    plt.imshow((sst-np.median(im))/tsm,vmin=vmin,vmax=vmax,cmap=cmap,origin='lower',aspect='auto')
    plt.title('Start')
    plt.subplot(2,2,3)
    plt.imshow((om-np.median(im))/sm,cmap=cmap,vmin=vmin,vmax=vmax,origin='lower',aspect='auto')
    plt.title('Synthesis')
    plt.subplot(2,2,4)
    plt.imshow((im-om)/sm,cmap=cmap,vmin=vmin,vmax=vmax,origin='lower',aspect='auto')
